...
export const SimulationWorkbench = memo(function SimulationWorkbench({
  ...
  useBodyScrollLock(isMobileInput && (isScenarioSettingsOpen || Boolean(isInfoPanelOpen && selectedPreviewNode)))
  ...
  const runtimeInfoSheet = shouldRenderRuntimeInfo ? (
    ...
    >
      <section
        className={`${isMobileInput ? 'flex max-h-[50dvh] w-screen flex-col rounded-t-2xl border-x-0 border-b-0 border-t' : 'h-screen w-[430px] max-w-[92vw] border-r'} overflow-hidden shadow-2xl ${
          isDark ? 'border-slate-700 bg-slate-950 text-slate-100' : 'border-slate-200 bg-white text-slate-950'
        }`}
        onClick={(event) => event.stopPropagation()}
      >
  ...
  const scenarioSettingsFab = !isScenarioSettingsOpen && !isFullscreen ? (
    <button
      ...
      aria-label="시나리오세팅"
      title="시나리오세팅"
      className={`fixed bottom-5 right-8 z-[120] flex h-12 w-12 items-center justify-center rounded-full border shadow-xl backdrop-blur transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${floatingSystemButtonClassName} ${scenarioSelectionPulseClassName}`}
    >
  ...
  const updateFullscreenLiveZoomLabel = useCallback((nextZoom: number) => {
    fullscreenZoomControlsRef.current?.setPercentLabel(`${Math.round(nextZoom * 100)}%`)
  }, [])
        ...
        <div className={panelClassName}>
          ...
          {runtimeScenarioPrompt ?? (
            <SimulationLayoutPreview
              ...
              onFullscreenLiveZoomChange={updateFullscreenLiveZoomLabel}
