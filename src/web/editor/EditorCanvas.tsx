...
export const EditorCanvas = memo(function EditorCanvas({
    ...
    if (hasVisibleBaseGround && isPointInsideRectBounds(cursor, baseGroundBounds)) {
      ...
      if (baseGroundNode) {
        ...
        if (event.pointerType === 'touch' || event.pointerType === 'pen' || isMobileInput) {
          openMobileNodeActionMenu(baseGroundNode, cursor, event.clientX, event.clientY)
        } else {
  ...
  const editorInfoSheet = isMobileInput && isEditorInfoPanelOpen ? (
    ...
    >
      ...
      >
        ...
        <div className="h-[calc(env(safe-area-inset-bottom)+40px)] shrink-0" aria-hidden="true" />
      </section>
    </div>
  ) : null
  ...
  const relationPreviewSheet = relationPreviewNode ? (
    <div
      className="fixed bottom-0 left-0 right-0 top-[var(--app-visual-offset-top,0px)] z-[235] flex h-[var(--app-visual-height,100dvh)] items-end bg-slate-950/55"
      role="dialog"
      aria-modal="true"
      aria-labelledby="relation-preview-sheet-title"
    >
      <section
        ref={mobileRelationPreviewSheetRef}
        className={`max-h-[calc(var(--app-visual-height,100dvh)-16px)] w-screen overflow-hidden rounded-t-2xl border-t shadow-2xl ${
          isDark ? 'border-slate-800 bg-slate-950 text-slate-100' : 'border-slate-200 bg-white text-slate-900'
        }`}
        onClick={(event) => event.stopPropagation()}
      >
        ...
        <header className={`flex items-center justify-between gap-3 border-b px-5 py-4 ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
          <div>
            <h2 id="relation-preview-sheet-title" className="text-base font-black">관계형성</h2>
            <p className={`mt-1 text-xs font-bold ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              {relationPreviewMode === 'child'
        ...
        {isMobileInput && !isEditorSettingsOpen ? (
          <div className="fixed bottom-5 right-8 z-[120] flex flex-col items-center gap-2">
            <button
              ...
              aria-label="편집 세팅"
              title="편집 세팅"
              className={`flex h-12 w-12 items-center justify-center rounded-full border shadow-xl backdrop-blur transition focus-visible:outline-none focus-visible:ring-2 ${floatingSystemButtonClassName}`}
            >
              <GearIcon />
            </button>
            <button
              ...
              title="객체 추가"
              className={`flex h-12 w-12 items-center justify-center rounded-full border shadow-xl backdrop-blur transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950 ${floatingBlueButtonClassName}`}
            >
              <PlusIcon />
            </button>
          </div>
